<template>
	<div class="p-3 bg-light border">
		<BRow class="">
			<BCol cols="12">
				<h4 class="text-primary">Check List</h4>
			...
			<BCol
				v-for="(question, i) in value.pump_checkListQuestions" :key="i"
				cols="12"
				class="mb-3"
			>
				...
				<div v-if="question.type == 'ynan'">
					<label v-for="(o, ii) in question.options" :key="ii" class="mx-3">
						<input
							v-model="value.pump_checkListAnswers[i]"
							type="radio"
							:name="question.name"
							:value="o"
						>
				...
				<div v-if="question.type == 'text-input'">
					<BFormInput v-model="value.pump_checkListAnswers[i]" />
				</div>
