...
module.exports = {
	...
	c_update: async ({ user_id, first_name, last_name, img_url, bio }) => {
		try {
			// [VALIDATE] user_id //
			...
			if (!validator.isAscii(first_name)) {
				return {
					executed: true,
					status: false,
					message: 'UserCollection: Invalid first_name'
				}
			}
			...
			if (!validator.isAscii(last_name)) {
				return {
					executed: true,
					status: false,
					message: 'UserCollection: Invalid last_name'
				}
			}
	...
	c_register: async ({ username, email, first_name, last_name, password }) => {
		try {
			// [VALIDATE] username //
	...
	// [READ-ALL] Sorted (No password) //
	c_readSelect: async ({ user_id, select = '_id first_name last_name profile_img bio verified location friends blocked username email created_at' }) => {
		try {
			// [VALIDATE] user_id //
			if (!mongoose.isValidObjectId(user_id)) {
				return {
	...
	/******************* [FUZZY-SEARCH] *******************/
	c_fuzzySearch: async (user_id, query, limit, skip) => {
		try {
			// [INIT] //
			let exactMatchInUsers = false

			// [SANTIZE] //
			limit = parseInt(limit)
			skip = parseInt(skip)

			// [INIT][MODIFY] Fuzzy search does not support '@' //
			const modedQuery = query.replace('@', '')
			...
			// [VALIDATE] modedQuery //
			if (!validator.isAscii(modedQuery)) {
				return {
					executed: true,
					status: false,
					message: 'usersCollection: Invalid modedQuery',
					existance: false,
				}
			}

			// [VALIDATE] exactMatchQuery //
			if (!validator.isAscii(exactMatchQuery)) {
				return {
					executed: true,
					status: false,
					message: 'usersCollection: Invalid exactMatchQuery',
					existance: false,
				}
			}
			...
			// [SEARCH] //
			let users = await UserModel.fuzzySearch({ query: modedQuery })
				.select('-password')
				.limit(limit)
				.skip(skip)
				.exec()
			...
			// [READ] Exact match //
			let exactMatch = await UserModel.findOne({ email: exactMatchQuery })
				.select('-password')
				.exec()

			//console.log(modedQuery, exactMatchQuery, users, exactMatch)

			// Flag if exact math within users
			users.forEach(user => {
				if (exactMatch) {
					if (JSON.stringify(user._id) == JSON.stringify(exactMatch._id)) {
						exactMatchInUsers = true
					}
				}
			})

			// Push exact match if its not already in "users"
			if (exactMatch && !exactMatchInUsers) { users.push(exactMatch) }

			// [FILL-DATA] //
			for (let i = 0; i < users.length; i++) {
				users[i].in_organization = false				
			}
