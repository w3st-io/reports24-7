...
class Auth {
	...
	static managerToken() {
		return (req, res, next) => {
			// [INIT] //
			const token = req.headers.manager_authorization
				...
				if (validator.isJWT(tokenBody)) {
					...
					jwt.verify(tokenBody, secretKey, async (err, decoded) => {
						try {
							if (decoded) {
								// [INIT] Put decoded in req //
								req.manager_decoded = decoded

								// Check verified //
								const verified = await managersCollection.c_verifiedStatus(
									decoded.manager_id
								)

								if (verified.status) { next() }
								else { res.send(verified) }
							}
							else {
								res.send({
	...
	// [MANAGER-TOKEN] NOT required //
	static managerTokenNotRequired() {
		return async (req, res, next) => {
			// [INIT] //
			const token = req.headers.manager_authorization
	...
	// [MANAGER-TOKEN] Verification NOT required //
	static managerTokenByPassVerification() {
		return (req, res, next) => {
			// [INIT] //
			const token = req.headers.manager_authorization
