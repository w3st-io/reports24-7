...
async function sendManagerVerificationMail(to, manager_id, VCode) {
	try {
		// [VALIDATE] //
		if (
			!validator.isEmail(to) ||
			!mongoose.isValidObjectId(manager_id) ||
			!validator.isAscii(VCode)
		) {
			return {
				executed: true,
		...
		const mailOptions = {
			...
			html: `
				<h1>Thank you creating an account! Verify & Join us!<h1/>
				<a href="${baseURL}/manager/verify/${manager_id}/${VCode}">
					<button>Click to Verify</button>
				</a>
			`
		}
...
// [PASSWORD-RESET] //
async function sendManagerPasswordResetEmail(to, manager_id, VCode) {
	try {
		// [VALIDATE] //
		if (
			!validator.isEmail(to) ||
		...
		const mailOptions = {
			...
			html: `
				<h1>Click the Link Below to Reset Your Password<h1/>
				<h4>If you did not request to change your password ignore this email</h4>
				<a href="${baseURL}/manager/password/reset/${manager_id}/${VCode}">
					<button>Click to Reset Password</button>
				</a>
			`
		}
